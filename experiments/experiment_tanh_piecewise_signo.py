# experiments/experiment_tanh_piecewise_signo.py
import os
import numpy as np
import pandas as pd
...
# ==============================================================
# Experimento: "Signo con tanh" usando Engine Piecewise (AUTO-CALIBRADO)
# Objetivo: repetir el setup del experimento original,
#           pero SIN perillas manuales (sigma_known/sigma_border):
#           - U = tanh(mu) dentro del engine
#           - régimen definido por percentiles de σ_train (auto-calibración)
#           - anti-falsos-positivos por signo en régimen known
# ==============================================================
...
gp = GaussianProcessRegressor(
    ...
    alpha=1e-8,          # casi sin ruido -> σ tiende a cero en train
    normalize_y=False
)
gp.fit(X_train, y_train)

# Auto-calibración de umbrales de régimen desde sigma en training 
...
# Motor QiCore Piecewise (base equivalente: eta y lambda_2)
qicore = QiCoreEnginePiecewise(
    eta=0.4,
    lambda_2=2.5,
    threshold=1.0,

    # IMPORTANTÍSIMO: ahora vienen auto-calibrados desde σ_train
    sigma_known=sigma_known,
    sigma_border=sigma_border,

    # amortiguación en frontera
    beta_neg_border=0.20,

    # en OOD manda sigma; misión se ignora (por defecto 0.0)
    mission_weight_ood=0.0
)

THRESHOLD = qicore.threshold

# Hamiltoniano usando SOLO el engine (sin tanh hardcodeada aquí)
rows = []
...
# Extra: distribución de regímenes dentro de zona conocida (auditoría)
regime_counts_known = df.loc[in_known, "regime"].value_counts(dropna=False).to_dict()

# Guardar CSV
csv_path = os.path.join(RESULTS_DIR, "experiment_intervalo_tanh_signo_piecewise.csv")
...
ax1.set_title("GP: μ(x) y σ(x) — Intervalo entrenado (negativo amplificado)")
ax1.legend()
...
ax2.set_title("QiCore Piecewise: H(x) — objetivo: corregir sesgo por signo en régimen known")
ax2.set_xlabel("x")
ax2.set_ylabel("H")
ax2.legend()
...
plt.tight_layout()
png_path = os.path.join(RESULTS_DIR, "experiment_intervalo_tanh_signo_piecewise.png")
plt.savefig(png_path, dpi=200)
print(f"PNG guardado en: {png_path}")
plt.show()

# Resumen
summary_path = os.path.join(RESULTS_DIR, "experiment_intervalo_tanh_signo_piecewise_summary.txt")
with open(summary_path, "w", encoding="utf-8") as f:
    f.write("=== EXPERIMENTO: tanh con engine piecewise (AUTO-CALIBRADO) ===\n\n")
